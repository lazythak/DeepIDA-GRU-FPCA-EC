...
def prepareTranscriptomicsSummary(filename):
    ...
    for i in range(len(dff['data_type'].to_list())):
        # if dff['data_type'].to_list()[i] == 'metatranscriptomics':
        if True:
...
def main():
    ...
    # filename = 'hmp2_mtx_pathabundance.tsv'
    ...
    dic,label_dict = prepareTranscriptomicsSummary('hmp2_metadata.csv')
    # print(dic)
    ...
    with open('parrot_metagenome_lmm.pkl', 'wb') as f:
        pickle.dump([subject_data, subject_label, names], f)
