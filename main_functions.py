...
# Original Author: Jiuzhou Wang and Professor Sandra Safo (https://arxiv.org/abs/2111.09964) --- (https://github.com/lasandrall/DeepIDA.git)
# Date: Nov 20, 2021
...
def DeepIDA_nonBootstrap(data_train, data_valid, data_test, Y_train, Y_valid, Y_test, 
                         structure, TS_num_features, islong = [], long_structure = [1, 128, 128], lr_rate = 0.01, n_epoch = 50):
    ...
    optimizer = torch.optim.Adam(params, lr = lr_rate, weight_decay = 1e-5)
    losses_train = []
    losses_valid = []
    ...
    for i in range(num_of_views):
        ...
        Deep    IDA_train_acc.append(train_acc)
...
def DeepIDA_VS(data_all, Y, n_boot, structure, n_epoch, variables_name_list, islong, long_structure):
    ...
    # outputs = variable_select_bs(data_all,Y,0,0,structure, n_epoch, islong, long_structure)
    tac = timer()
...
def DeepIDA_Bootstrap(data_train, data_valid, data_test, Y_train, Y_valid, Y_test, 
                         structure,n_boot, n_epoch_boot, variables_name_list, TS_num_features, islong = [], long_structure = [1, 128, 128], lr_rate = 0.01, n_epoch_nonboot = 50 ):
    ...
    VS_result = DeepIDA_VS(data_VS, Y_VS, n_boot, structure, n_epoch_boot, variables_name_list, islong, long_structure)
    ...
    print(data_test_s[2])
    # final = DeepIDA_nonBootstrap(data_train_s, data_valid_s, data_test_s, Y_train, Y_valid, Y_test, structure, [5, 5, 5],[0, 1, 1], [3,256,20], lr_rate, n_epoch_nonboot)
